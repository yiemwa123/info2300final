...
if (isset($_POST["addmovie"])) {
    if (!empty($title) && !empty($year) && !empty($rating) && !empty($synopsis) && !empty($taginput)&& !empty($poster["name"])) {
        try {
            $sql = "INSERT INTO movies (movie_name, year, rating, synopsis) VALUES (:moviename, :year, :rating, :synopsis);";
            $params = array(':moviename' => $title, ':year' => $year, ':rating' => $rating, ':synopsis' => $synopsis);
            ...
            if ($poster['error'] == UPLOAD_ERR_OK) {
                ...
                $lastimageid = $db->lastInsertId();
                $sql = "UPDATE movies SET image_id=:lastid WHERE movie_name=:moviename;";
                $params = array(':moviename' => $title, ':lastid' => $lastimageid);
                exec_sql_query($db, $sql, $params);
...
<body>
    ...
    <?php
        ...
        if ($showsuccess) {
            echo  "<h3 class='works'> Success! </h3>";
        }
    } ?>
