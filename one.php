...
$sql = "SELECT movies.movie_name, images.image_ext, tags.tag_name, movies.image_id, movies.year, movies.rating, movies.synopsis, tags.id FROM movies INNER JOIN images ON movies.image_id=images.id INNER JOIN imagetotag ON images.id=imagetotag.image_id INNER JOIN tags ON tags.id=imagetotag.tag_id WHERE (movies.id=:id);";
...
if ($result) {
    ...
    foreach ($records as $record) {
        ...
        if (isset($_POST[$deleteid])) {
            $sql = "DELETE FROM imagetotag WHERE tag_id=:counter;";
            $params = array(":counter" => $deleteid);
    ...
    if (isset($_POST['addtag'])) {
        ...
        foreach ($tags as $tag) {
            ...
            try {
                ...
                $sql = "SELECT imagetotag.image_id, tags.id FROM tags INNER JOIN imagetotag ON imagetotag.tag_id=tags.id WHERE tags.id=:tag AND imagetotag.image_id=:imgid;";
                $params = array(':tag' => $tag_id, ":imgid"=>$img);
...
<body>
    ...
    if (!$result) {
        echo "<h2>Movie doesn't exist</h2>";
    }
